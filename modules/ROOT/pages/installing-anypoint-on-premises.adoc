...
* link:{attachmentsdir}/anypoint-platform-1.1.1.zip[Anypoint Platform On Premise Edition]: For installing the entire functionality of the Anypoint Platform, including API administration tools.
...
[TIP]
For a more detailed explanation of Docker, see the https://docs.docker.com/engine/understanding-docker/[Docker documentation].
...
http://www.docker.com[Docker] runs natively on Linux, including Debian, Ubuntu, Red Hat, CentOS, Oracle and others.
...
* https://docs.docker.com/engine/installation/[Linux] (check the link for supported distributions)
* https://docs.docker.com/engine/installation/mac/[OS X] (10.6 "Snow Leopard" and above)
* https://docs.docker.com/engine/installation/windows/[Windows] (7 and 8.1)
...
Follow the on-screen prompts.  Notice that once executed a new file will be generated from this process named `truststore.jks)`.  This file is required by xref:3.8@mule-runtime::mule-agents.adoc[the Mule Agent] for all runtimes you wish to register to the console because the agent uses 2-way SSL validation.  Before registering a runtime, you must copy this file to the `conf/` folder of that runtime.
...
For details on managing API Platform, see xref:access-management::index.adoc[Anypoint Platform Administration].
...
[TIP]
For a complete overview of the 'pg_dump' command, see http://www.enterprisedb.com/docs/en/8.4/pg/app-pgdump.html[PostgreSQL's documentation].
...
To add a server to your on-premises Anypoint Platform, you need to http://mule-agent.s3.amazonaws.com/1.3.1/mule-agent-1.3.1.zip[download] and install the agent.
...
For a full description of the steps outlined in this section, see xref:runtime-manager::managing-servers.adoc#add-a-server[Managing Servers].
