...
[WARNING]
====
Before running the installer, ensure that your system meets the minimum system requirements listed in xref:system-requirements.adoc[About Minimum System Requirements]. If your system does not meet these requirements, you cannot install Anypoint Platform Private Cloud Edition.
====
...
. Obtain the `anypoint-1.5.2-installer.tar.gz` archive from your customer support representative.
...
. Select an installation flavor and register each of your servers as nodes.
+
...
+
	* **1-node configuration**: installs all platform components run on a single node. This configuration is intended for demo purposes only and has lower hardware requirements. It is not supported for development or production environments. The demo installation includes its own LDAP server to use for testing. See xref:demo-ldap-server.adoc[Demo LDAP Server] for details on what it contains and how to use it.
...
+
See xref:system-requirements.adoc#supported-top[Supported Network Topologies] for more information about the 3-node and 6-node configurations.
+
After installation, you can add or remove nodes as necessary using the Ops Center.
+
. Register each of your servers as nodes.
+
image::installation-on-prem4.png[]
+
. Run the command displayed in **Add existing server** from the command line of each node in your installation.
+
...
. Configure IP addresses and devices.
+
...
+
For your devices, you can either leave the default settings, or you can set up your own, see xref:system-requirements.adoc#device-requirements[Device Requirements] for details. Ensure that on each of your nodes, both devices are configured with enough space.

. After all of your nodes are visible and configured, click *Start Installation*.
+
...
+
Platform users sign in to accounts that belong to the organization name that you provide here or a business group within it. See xref:access-management::organization.adoc[Organizations] for more information.
