...
. Since the MarketingAdmin LDAP group is configured in this test organization as having the xref:access-management::roles.adoc[Roles] `Exchange administrator` and `Application Admin (Production)`. You can also verify that the logged in user actually is an exchange administrator by carrying out tasks in Anypoint Exchange. The same applies for the Application Admin role, which should let you deploy and manage applications on xref:runtime-manager::index.adoc[Runtime Manager].
