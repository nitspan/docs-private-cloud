...
After reviewing this topic and ensuring that your environment meets all of the minimum requirements, see xref:install-workflow.adoc[Workflow: Install Anypoint Platform Private Cloud Edition] for information on installing the platform.
...
Your network must include an SMTP server to manage e-mail alerts that are triggered by the platform. See xref:access-management::private-cloud-edition-features.adoc#smtp[Private Cloud Edition Features] for information on configuring SMTP for the Anypoint Platform.
