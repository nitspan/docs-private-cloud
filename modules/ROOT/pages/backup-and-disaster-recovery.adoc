...
. Determine which node is registered as a master node:
+
----
...
. Move the compressed archive file to: `/var/lib/gravity/planet/share`. You can transfer this file securely via the following command:
+
----
...
. Run the following script after the restore is complete. This script recreates each pod to ensure that it uses the restored secrets.
+
----
